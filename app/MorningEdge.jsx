                    ...
                    {(() => {
                      ...
                      return moves.length > 0 ? (
                        <ul className="space-y-1">
                          {moves.map((w, i) => (
                            <SmartMoneyRow key={i} item={w} onOpenLink={openLinkInBrowser} />
                          ))}
                        </ul>
                      ) : (
                  ...
                  <div className="rounded-xl border border-amber-200 bg-white/60 p-3.5">
                    ...
                    {(() => {
                      ...
                      return moves.length > 0 ? (
                        <ul className="space-y-1">
                          {moves.map((c, i) => (
                            <SmartMoneyRow key={i} item={c} onOpenLink={openLinkInBrowser} />
                          ))}
                        </ul>
                      ) : (
                  ...
                  <div className="rounded-xl border border-amber-200 bg-white/60 p-3.5">
                    ...
                    {(() => {
                      ...
                      return moves.length > 0 ? (
                        <ul className="space-y-1">
                          {moves.map((h, i) => (
                            <SmartMoneyRow key={i} item={h} onOpenLink={openLinkInBrowser} />
                          ))}
                        </ul>
                      ) : (
...
function SmartMoneyRow({ item, onOpenLink }) {
  ...
  // Build the URL we'll actually open. The model often returns home-page
  // URLs (e.g. "https://capitoltrades.com/") that don't show the specific
  // trade. Better: always route to a Google search of the trade text +
  // ticker so the user can find the actual source. The model's source_url
  // is preserved as a hint when it points to a specific filing page.
  const buildSearchUrl = () => {
    const query = `${text || ""} ${ticker || ""}`.trim();
    return `https://www.google.com/search?q=${encodeURIComponent(query)}`;
  };
  const isSpecificSource = (() => {
    if (!source_url || typeof source_url !== "string") return false;
    try {
      const u = new URL(source_url);
      // Specific filing pages have path segments — home pages don't
      return u.pathname && u.pathname.length > 4;
    } catch {
      return false;
    }
  })();
  const targetUrl = isSpecificSource ? source_url : buildSearchUrl();
  ...
  return (
    <li>
      <button
        ...
        onClick={(e) => {
          e.preventDefault();
          // Parent's openLinkInBrowser handles both window.open and the
          // confirmation modal. Do not call window.open here directly —
          // it would double-open the link.
          if (onOpenLink) {
            onOpenLink(targetUrl);
          } else if (typeof window !== "undefined") {
            // Standalone fallback only when no parent handler provided
