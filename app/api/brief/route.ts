...
// Extract a Layer-A-shaped slice from a full brief generated by the legacy
// path. This lets Tier 3 populate Layer A as a side effect, so subsequent
// CSV updates within the same day get Tier 2's fast path even when cron
// hasn't run yet.
//
// We use the brief's radar_watch as radar_candidates. radar_watch is already
// filtered to exclude the user's holdings, but for Tier 2 reads later today,
// re-filtering against the same holdings will be a no-op. If holdings change,
// the stale items just won't appear (acceptable degradation).
function extractLayerAFromBrief(brief: any): any | null {
  ...
  // Use radar_watch as candidates (already filtered, slightly stale-ok)
  if (Array.isArray(brief.radar_watch)) slice.radar_candidates = brief.radar_watch;
  // Build a synthetic todays_edge_market from todays_edge if present
...
export async function POST(request: Request) {
  try {
    ...
    // Side effect: populate Layer A from this Tier 3 result so subsequent
    // CSV updates today get Tier 2's fast path.
    const synthLayerA = extractLayerAFromBrief(merged);
    if (synthLayerA) {
      await cacheWriteLayerA(dateKey, synthLayerA);
    }
...
- Empty arrays are acceptable; placeholder strings or padding-with-news are NEVER acceptable.`;
  ...
  return callJsonChunk(prompt, { search: true, maxTokens: 5000, maxSearches: 2 });
}
